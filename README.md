...
# Notice
  ...
  - MA service default not enable if you want to used you should copy `/opt/rolling/rolling_ma_service/fcc-unlock.d` to `/usr/lib/x86_64-linux-gnu/ModemManager/` command :
    1. `cp -raf  /opt/rolling/rolling_ma_service/fcc-unlock.d  /usr/lib/x86_64-linux-gnu/ModemManager/`
...
1. cmake -S . -B build -DPROJECT_BUILD=xxx -DOEM_BUILD=yyy<br>
    ...
    `cmake -S . -B build -DPROJECT_BUILD=rw101 -DOEM_BUILD=lenovo` <br>
  if you want to install custom path you can send cmd:<br>
    `cmake -S . -B build -DPROJECT_BUILD=rw101 -DOEM_BUILD=lenovo --install-prefix <custm path>` <br>
